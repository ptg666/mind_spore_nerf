...
def test_net(img_h, img_w, focal, renderer, test_poses, gt=None, on_progress=None, on_complete=None):
    ...
    Args:
        ...
        on_progress (Optional[Callable]): a callback function invoked per generation of a result.
        on_complete (Optional[Callable]): a callback function invoked after generating all results.

    Returns:
        Tuple[ndarray, ndarray, ndarray], time sequence, loss sequence, and psnr sequence.
    """
    ...
    with tqdm(test_poses) as p_bar:
        for j, test_pose in enumerate(p_bar):
            ...
            rgb_map, _ ,depth_map = renderer(test_batch_rays)
            
            # import time
            print(len(depth_map))
            print(len(rgb_map))
            depth_map = reshape_op(depth_map, (img_h, img_w))
            depth_map = depth_map.asnumpy()
