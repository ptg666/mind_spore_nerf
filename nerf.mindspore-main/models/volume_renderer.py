...
class VolumeRenderer(nn.Cell):
    ...
    def construct(self, rays):
        ...
        res_ls = {"rgb_map_coarse": [], "rgb_map_fine": [],"depth_map_coarse":[],"depth_map_fine":[]}
        ...
        for i in range(0, cap_n_rays, self.chunk):
            ...
            rgb_map_coarse, weights_coarse, depth_map_coarse = self._composite(optic_d, s_samples, ray_dirs)
            ...
            if self.cap_n_importance > 0:
                ...
                rgb_map_fine, _ ,depth_map_fine= self._composite(optic_d, z_vals, ray_dirs)
            else:
                rgb_map_fine = rgb_map_coarse
                depth_map_fine = depth_map_coarse
            res_ls["rgb_map_coarse"].append(rgb_map_coarse)
            res_ls["rgb_map_fine"].append(rgb_map_fine)
            res_ls["depth_map_coarse"].append(depth_map_coarse)
            res_ls["depth_map_fine"].append(depth_map_fine)
        ...
        return res["rgb_map_fine"], res["rgb_map_coarse"],res["depth_map_fine"]
    ...
    def _composite(self, optic_d, s_samples, rays_d):
        ...
        # depth map
        depth_map = sum_op(weights * s_samples,-1)
        if self.white_bkgd:
            rgb_map = rgb_map + (1.0 - acc_map[..., None])
        print("depth_shape :{}".format(depth_map.shape))
        print("rgb_shape :{}".format(rgb_map.shape))
...
class VolumeRendererForInference():
    ...
    def __call__(self, rays):
        ...
        res_ls = {"rgb_map_coarse": [], "rgb_map_fine": [],"depth_map_coarse":[]}
        ...
        return res["rgb_map_fine"], res["rgb_map_coarse"],res["depth_map_coarse"]
